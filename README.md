# Makstore eBay → OnBuy Sync (manual-OnBuy variant)
...
1. An employee adds a row: **SKU** (its **digits must form a real,
   ...
   are ignored, e.g. `MAK-5012345678900`; the digits become the product's
...
4. **Creating products on OnBuy**: run the "Export OnBuy Upload" workflow
   from the Actions tab and download the `makstore-onbuy-upload` artifact — a
...
**Google Sheet** — name it `Makstore_Feed_Master` (or set the `SHEET_NAME`
...
`SUPABASE_FEED_BUCKET` (optional, e.g. `makstore-feeds` — create a public
...
```sql
create table "Makstore_Feed_Master" (
...
```sql
alter table "Makstore_Feed_Master"
  add column "Variant Choice" text,
  add column "Variant Group" text,
  add column "Variant" text;
```

The code already defaults to this table name (`Makstore_Feed_Master`) — only
