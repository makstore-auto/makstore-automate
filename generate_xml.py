...
# ================= CONFIG =================
# MAKSTORE VARIANT of the OnBuy-eBay pipeline. Key difference from the
...
SHEET_NAME = os.getenv("SHEET_NAME") or "Makstore_Feed_Master"
...
# ================= ONBUY API PUSH (permanently OFF for Makstore) =================
# The Makstore store updates OnBuy manually via CSV export - by explicit policy,
...
def sku_numeric_part(sku):
    ...
    the barcode ("MAK-5012345678900") and everything validated, exported or
...
def main():
    ...
    if missing or duplicates:
        ...
        notify.send_alert_email("Makstore sheet header row needs fixing", message)
        sys.exit(1)
    ...
    # "MAK-5012345678900" and "5012345678900" count as duplicates. The FIRST
    ...
    for idx, row in batch:
        ...
        # ================= CHANGE ALERTS (Makstore manual-update workflow) =====
        ...
        # Change-alert columns (Makstore manual-update workflow). "Applied on
        ...
        supabase_row = {
            ...
            # Makstore change-alert state, mirrored so Supabase holds the same
    ...
    # ================= CHANGE-ALERT EMAIL (Makstore manual-update workflow) =================
    ...
    if change_log:
        notify.send_alert_email(
            f"Makstore: {len(change_log)} product change(s) need action on OnBuy",
            "These products changed at the supplier and need a manual update on OnBuy:\n\n"
            + "\n".join(f"- SKU {sku}: {note}" for sku, note in change_log)
            + "\n\nThe same rows are highlighted in the Makstore sheet (amber = change pending, "
